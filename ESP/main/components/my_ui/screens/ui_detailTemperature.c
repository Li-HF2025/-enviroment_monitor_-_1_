...
void ui_detailTemperature_screen_init(void)
{
    ...
    // 标题
    lv_obj_t * title = lv_label_create(ui_detailTemperature);
    lv_obj_set_pos(title, 0, 10);
    lv_obj_set_align(title, LV_ALIGN_TOP_MID);
    lv_label_set_text(title, "Temperature Sensor");
    lv_obj_set_style_text_font(title, &lv_font_montserrat_22, LV_PART_MAIN | LV_STATE_DEFAULT);

    // 开关卡片
    lv_obj_t * card = lv_obj_create(ui_detailTemperature);
    ...
    ui_tempSwitchLabel = lv_label_create(card);
    lv_obj_set_pos(ui_tempSwitchLabel, -30, 0);
    lv_obj_set_align(ui_tempSwitchLabel, LV_ALIGN_LEFT_MID);
    lv_label_set_text(ui_tempSwitchLabel, "Sensor: ON");
    lv_obj_set_style_text_font(ui_tempSwitchLabel, &lv_font_montserrat_20, LV_PART_MAIN | LV_STATE_DEFAULT);

    ui_Switch1 = lv_switch_create(card);
